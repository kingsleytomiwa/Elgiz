...
const FormContent = <T extends unknown>({ lang, categories, ...tab }: TabPanelProps<T> & { lang: string; categories?: FoodCategory[]; }) => {
            ...
            <Select<string>
              ...
              renderValue={() =>
                <Typography sx={{ color: formik.values.categoryId ? "black" : "#676E76", fontSize: "14px", fontWeight: 500 }}>
                  {formik.values.categoryId === "" ? t("category") : (categories?.find(field => field.id === formik.values.categoryId)?.name as any)?.[i18next.language]}
                </Typography>
              }
              endAdornment={
                <Button
